...
GTF_STRINGTIE="${STAR_INDEX_DIR}/annotation_stringtie.gtf"
grep -v $'\tgene\t' ${GTF_FILE} > ${GTF_STRINGTIE}
echo "原始GTF行数: $(wc -l < ${GTF_FILE})"
echo "过滤后行数: $(wc -l < ${GTF_STRINGTIE})"
echo ""
...
# 清理GTF文件（移除gene_id为空的行）
echo "准备featureCounts专用GTF（清理空gene_id）..."
GTF_CLEAN="${STAR_INDEX_DIR}/annotation_clean.gtf"
awk '$0 !~ /gene_id "";/' ${GTF_FILE} > ${GTF_CLEAN}
echo "原始GTF行数: $(wc -l < ${GTF_FILE})"
echo "清理后行数: $(wc -l < ${GTF_CLEAN})"
echo ""

# 准备所有 BAM 文件列表
BAM_FILES=$(ls ${BAM_DIR}/*/*-Aligned.sortedByCoord.out.bam 2>/dev/null | tr '\n' ' ')

# 检查是否找到BAM文件
if [ -z "${BAM_FILES}" ]; then
    echo "错误: 未找到BAM文件"
    echo "检查路径: ${BAM_DIR}/*/*-Aligned.sortedByCoord.out.bam"
    exit 1
fi

echo "找到 $(echo ${BAM_FILES} | wc -w) 个BAM文件"
